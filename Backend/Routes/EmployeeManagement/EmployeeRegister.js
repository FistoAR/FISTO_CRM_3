...
router.post("/", uploadFields, (req, res) => {
  console.log("POST /api/employeeRegister called");
...
router.put("/:id", uploadFields, (req, res) => {
  console.log(`PUT /api/employeeRegister/${req.params.id} called`);
  const { id } = req.params;
  const data = req.body;
  ...
  db.pool.query(selectQuery, [id], (err, results) => {  // ← CHANGED
    if (err) {
    ...
    try {
      existingIds = existingEmployee.ID_url ? JSON.parse(existingEmployee.ID_url) : {};
      existingCerts = existingEmployee.Certificates_url ? JSON.parse(existingEmployee.Certificates_url) : {};
      existingOthers = existingEmployee.otherDocs_url ? JSON.parse(existingEmployee.otherDocs_url) : [];
    } catch (parseErr) {
      console.error("JSON parse error:", parseErr);
    }

    const finalIdsData = { ...existingIds, ...newIdsData };
    const finalCertsData = { ...existingCerts, ...newCertificatesData };
    const finalOthersData = newOtherDocsData.length > 0 ? newOtherDocsData : existingOthers;

    const profileUrl = req.files?.profile 
      ? `/Images/profiles/${req.files.profile[0].filename}` 
      : existingEmployee.profile_url;
    const resumeUrl = req.files?.resume 
      ? `/Images/resumes/${req.files.resume[0].filename}` 
      : existingEmployee.resume_url;

    const updateQuery = `
      UPDATE employees_details SET
        employee_name = ?, dob = ?, gender = ?,
        email_personal = ?, email_official = ?, phone_personal = ?, phone_official = ?,
        designation = ?, team_head = ?, employment_type = ?, working_status = ?,
        join_date = ?, intern_start_date = ?, intern_end_date = ?, duration_months = ?,
        address = ?, profile_url = ?, resume_url = ?,
    ...
    db.pool.query(  // ← CHANGED
      ...
      [
        ...
        JSON.stringify(finalCertsData),
        JSON.stringify(finalOthersData),
        id,
      ],
      (err, result) => {
        if (err) {
