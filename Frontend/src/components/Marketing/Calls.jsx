  ...
  const fetchCounts = async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/followups/counts?employee_id=${employeeId}`
      );
  ...
  const fetchClients = async () => {
    ...
    try {
      ...
      if (mainTab === "clientsData") {
        if (subTab === "deleted") {
          url = `${API_URL}/clientAdd?active=false&employee_id=${employeeId}`;
        } else if (subTab === "current") {
          url = `${API_URL}/clientAdd?employee_id=${employeeId}`;
        }
      } else if (mainTab === "followups") {
        if (subTab === "first_followup") {
          url = `${API_URL}/followups?status=first_followup&employee_id=${employeeId}`;
        } else if (subTab === "second_followup") {
          url = `${API_URL}/followups?status=second_followup&employee_id=${employeeId}`;
        } else if (subTab === "not_available") {
          url = `${API_URL}/followups?status=not_available&employee_id=${employeeId}`;
        } else if (subTab === "not_interested") {
          url = `${API_URL}/followups?status=not_interested&employee_id=${employeeId}`;
        } else if (subTab === "droped") {
          url = `${API_URL}/followups?status=droped&employee_id=${employeeId}`;
        }
      } else if (mainTab === "Leads") {
        url = `${API_URL}/followups?status=converted&employee_id=${employeeId}`;
      }else if (mainTab === "marketing_leads") {
        url = `${API_URL}/followups?status=converted`;
      }
  ...
  return (
    <div className="text-black min-h-[92%] max-h-[100%] w-[100%] max-w-[100%] overflow-hidden">
      <div className="w-[100%] h-[91vh] flex flex-col gap-[1vh]">
        <div className="bg-white flex justify-between overflow-hidden rounded-xl shadow-sm h-[6%] flex-shrink-0">
          <div className="flex border-b border-gray-200 h-full w-full">
            ...
            {type !== "Marketing" && (
              <button
                onClick={() => {
                  setMainTab("marketing_leads");
                }}
                className={`px-[1.5vw] cursor-pointer font-medium text-[0.9vw] transition-colors ${
                  mainTab === "marketing_leads"
                    ? "border-b-2 border-black text-black"
                    : "text-gray-600 hover:text-gray-900"
                }`}
              >
        ...
        {( mainTab !== "Leads" && mainTab !== "eixsting_clients" && mainTab!=="marketing_leads" ) && (
