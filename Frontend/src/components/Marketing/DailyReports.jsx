...
const isTaskOverdue = (task, timeRange) => {
  ...
  // For Today tab - check deadline_time only
  if (timeRange === "today") {
    const deadlineTime = task.deadline_time || task.emp_deadline_time;

    if (deadlineTime === "MORNING") {
      // Morning deadline is 1:30 PM (13:30)
      const morningDeadline = 13 * 60 + 30; // 810 minutes
      return currentTime > morningDeadline;
    } else if (deadlineTime === "EVENING") {
      // Evening deadline is 6:30 PM (18:30)
      const eveningDeadline = 18 * 60 + 30; // 1110 minutes
  ...
  // For Weekly and Monthly - check deadline_date AND deadline_time
  if (timeRange === "weekly" || timeRange === "monthly") {
    ...
    if (deadlineDay < today) {
      return true; // Past the deadline date entirely
    }

    // If it's the deadline day, check the time
    if (deadlineDay.getTime() === today.getTime()) {
      if (deadlineTime === "MORNING") {
        const morningDeadline = 13 * 60 + 30; // 1:30 PM
        return currentTime > morningDeadline;
      } else if (deadlineTime === "EVENING") {
        const eveningDeadline = 18 * 60 + 30; // 6:30 PM
        return currentTime > eveningDeadline;
      }
    }
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const body = {
      ...
      submitted_datetime: currentDateTime, // Send IST datetime
    };
  ...
  // header title
  const headerTitle = `${(task?.timeLabel || "Today").replace(/\b\w/g, (c) =>
    c.toUpperCase()
  )} ${task?.index ?? 1} - ${(task?.timeLabel || "Today").replace(
    /\b\w/g,
    (c) => c.toUpperCase()
  )}`;
...
const Report = () => {
  ...
  // Format deadline display based on time range
  // Inside the Report component, replace the getDeadlineDisplay function:

  const getDeadlineDisplay = (task) => {
    if (timeRange === "today") {
      // Today: Show only time (Morning/Evening)
      return task.deadline_time || "N/A";
    } else {
      // Weekly/Monthly: Show both date and time
      const deadlineDate = task.deadline_date
        ? formatDate(task.deadline_date)
        : "N/A";
      const deadlineTime = task.deadline_time || "N/A";

      if (deadlineDate === "N/A" && deadlineTime === "N/A") {
        return "N/A";
      }
      ...
      return (
        <div className="flex flex-col items-center gap-[0.2vw]">
          ...
          <span className="text-[0.75vw] text-gray-600">({deadlineTime})</span>
        </div>
      );
    }
  };
                    ...
                    {paginatedTasks.map((task, index) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* Updated Deadline Column */}
                          <td className="px-[0.7vw] py-[0.5vw] text-[0.85vw] border border-gray-300">
                            <div className="flex items-center justify-center gap-[0.4vw]">
                              {timeRange === "today" ? (
                                // Today tab: Show only time with overdue indicator
                                <>
                                  <span
                                    className={
                                      overdue
                                ...
                                // Weekly/Monthly: Show date and time in column format
