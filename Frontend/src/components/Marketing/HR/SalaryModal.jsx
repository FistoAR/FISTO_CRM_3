...
const SalaryModal = ({
  ...
  useEffect(() => {
    if (currentEmployee) {
      setFormData({
        ...
        month: selectedMonthYear.month || new Date().getMonth() + 1,
        total_leave_days: currentEmployee.total_leave_days,
        paid_leave_days: currentEmployee.paid_leave_days,
        incentive: currentEmployee.incentive,
        bonus: currentEmployee.bonus,
        medical: currentEmployee.medical,
        other_allowance: currentEmployee.other_allowance,
      });
    }
  }, [currentEmployee]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/salary-calculation/save-salary`, {
        method: "POST",
        headers: {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-[80vw] h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-[1.2vw] border-b border-gray-200 flex-shrink-0">
          <h2 className="text-[1.2vw] font-semibold text-gray-900">
            {currentEmployee?.salaryId ? "Update Employee Salary" : "Add Employee Salary"}
          </h2>
          <button
            onClick={() => {
              setShowSalaryModal(false);
        ...
        <div className="flex-1 overflow-y-auto p-[1.2vw] space-y-[1.5vw]">
          ...
          <div className="space-y-[0.8vw]">
            <div className="flex items-center gap-[0.5vw] mb-[0.8vw]">
              ...
              <h3 className="text-[1vw] font-semibold text-gray-800">Salary Information</h3>
            </div>
            <div className="grid grid-cols-3 gap-[1vw]">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleLeaveInput("total_leave_days", e.target.value)}
                  placeholder="0"
                  className="w-full px-[0.8vw] py-[0.5vw] border border-gray-300 rounded-lg text-[0.85vw]"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => handleLeaveInput("paid_leave_days", e.target.value)}
                  placeholder="0"
                  className="w-full px-[0.8vw] py-[0.5vw] border border-gray-300 rounded-lg text-[0.85vw]"
                />
              </div>
          ...
          <div className="space-y-[0.8vw]">
            <div className="flex items-center gap-[0.5vw] mb-[0.8vw]">
              ...
              <h3 className="text-[1vw] font-semibold text-gray-800">Allowances & Bonuses</h3>
            </div>
            <div className="grid grid-cols-4 gap-[1vw]">
              <div>
                <label className="block text-[0.85vw] font-medium text-gray-700 mb-[0.3vw]">
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-lg p-[1vw]">
            <div className="flex flex-col gap-[0.5vw]">
              <div className="grid grid-cols-3 gap-[1vw] text-[0.8vw] text-gray-700">
                <div>Total Days: <span className="font-semibold">{salaryBreakdown.totalDays}</span></div>
                <div>Working Days: <span className="font-semibold">{salaryBreakdown.workingDays}</span></div>
                <div>Sundays: <span className="font-semibold">{salaryBreakdown.sundays}</span></div>
                <div>Unpaid Leave: <span className="font-semibold text-orange-600">{salaryBreakdown.unpaidLeave}</span></div>
                <div>Per Day Salary: <span className="font-semibold">₹ {salaryBreakdown.perDaySalary.toFixed(2)}</span></div>
                <div>Total Deduction: <span className="font-semibold text-red-600">₹ {salaryBreakdown.deductionAmount.toFixed(2)}</span></div>
              </div>
              <div className="border-t-2 border-blue-300 pt-[0.5vw] mt-[0.5vw] flex justify-end">
                <span className="text-[1.3vw] font-bold text-blue-700">
                  Final Salary: ₹ {totalSalary.toLocaleString("en-IN", {
        ...
        <div className="flex justify-end gap-[0.8vw] p-[1.2vw] border-t border-gray-200 flex-shrink-0 bg-gray-50">
          ...
          >
            {currentEmployee?.salaryId ? "Update Salary" : "Save Salary"}
          </button>
        </div>
      </div>
    </div>
